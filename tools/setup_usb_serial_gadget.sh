...
# Pi 4 USB-C device/gadget mode:
# - Do NOT set otg_mode=1 (that enables XHCI host-only and hides DWC2 / empty UDC).
# - dtoverlay=dwc2,dr_mode=peripheral makes the Pi a device (PC sees COMx).
if grep -Eq '^[[:space:]]*otg_mode=' "$BOOT_CFG"; then
  sudo sed -i -E '/^[[:space:]]*otg_mode=/d' "$BOOT_CFG"
  echo "removed: otg_mode= (required for DWC2 gadget; otg_mode=1 is host-only XHCI)"
fi
if grep -Eq '^[[:space:]]*dtoverlay=dwc2' "$BOOT_CFG"; then
  sudo sed -i -E 's/^[[:space:]]*dtoverlay=dwc2.*/dtoverlay=dwc2,dr_mode=peripheral/' "$BOOT_CFG"
  echo "set: dtoverlay=dwc2,dr_mode=peripheral"
else
  ensure_line "$BOOT_CFG" "dtoverlay=dwc2,dr_mode=peripheral"
fi
